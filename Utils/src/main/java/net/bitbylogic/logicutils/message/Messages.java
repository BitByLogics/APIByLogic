...
public class Messages {
    ...
    public static String main(String prefix, String message, Object... replacements) {
        return replace("<c#primary>&l%s &8%s <c#secondary>%s", prefix, DOT, replace(message, applyHighlightColor(LogicColors.SECONDARY.toString(), LogicColors.HIGHLIGHT.toString(), replacements)));
    }

    public static String error(String prefix, String message, Object... replacements) {
        return replace("<c#error_primary>&l%s &8%s <c#error_secondary>%s", prefix, DOT, replace(message, applyHighlightColor(LogicColors.ERROR_SECONDARY.toString(), LogicColors.ERROR_HIGHLIGHT.toString(), replacements)));
    }

    public static String success(String prefix, String message, Object... replacements) {
        return replace("<c#success_primary>&l%s &8%s <c#success_secondary>%s", prefix, DOT, replace(message, applyHighlightColor(LogicColors.SUCCESS_SECONDARY.toString(), LogicColors.SUCCESS_HIGHLIGHT.toString(), replacements)));
    }

    public static String listHeader(String prefix, String info, Object... replacements) {
        return replace("<c#primary>&l%s &8%s <c#secondary>%s", prefix, DOT, replace(info, applyHighlightColor(LogicColors.SECONDARY.toString(), LogicColors.HIGHLIGHT.toString(), replacements)));
    }

    public static String listItem(String prefix, String info, Object... replacements) {
        return replace("&8| &8» <c#success_primary>%s &8%s <c#success_secondary>%s", prefix, DOT, replace(info, applyHighlightColor(LogicColors.SUCCESS_PRIMARY.toString(), LogicColors.SUCCESS_SECONDARY.toString(), replacements)));
    }

    public static String dottedMessage(String prefix, String info, Object... replacements) {
        return replace("<c#success_primary>%s &8%s <c#success_secondary>%s", prefix, DOT, replace(info, applyHighlightColor(LogicColors.SUCCESS_PRIMARY.toString(), LogicColors.SUCCESS_SECONDARY.toString(), replacements)));
    }

    public static void sendMessage(Player player, String prefix, String message, Placeholder... placeholders) {
        String formattedMessage = LogicColors.SUCCESS_PRIMARY + "&l" + prefix + " &8• " + LogicColors.SUCCESS_SECONDARY + message;
    ...
    public static String[] getPagedList(String header, List<String> data, int page) {
        ...
        for (int i = startingItem; i < lastItem; i++) {
            ...
            text.add(format("&8| &8» " + LogicColors.HIGHLIGHT.toString() + item));
        }
